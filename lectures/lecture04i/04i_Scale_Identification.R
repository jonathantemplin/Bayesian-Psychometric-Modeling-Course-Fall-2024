...
# building multiple standardized sum scores to start theta in the correct location
sumScores = as.matrix(conspiracyItems) %*% matrix(data = 1, nrow = ncol(conspiracyItems), ncol = 1)

# standardize sum scores to have mean 0 and standard deviation 1
thetaInit = as.numeric(scale(sumScores))

# set any missing values to a random number
thetaInit[which(is.na(thetaInit))] = rnorm(n = 1, mean = 0, sd = 1)
...
# building multiple standardized sum scores to start theta in the correct location
sumScores = as.matrix(conspiracyItems) %*% Qmatrix

# standardize sum scores to have mean 0 and standard deviation 1
thetaInit = scale(sumScores)
...
save.image(file = "lecture04i.RData")
